...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--start', type=str, default=0, help="When the curve starts.")
    parser.add_argument('-e', '--end', type=str, default=2*np.pi, help="When the curve ends.")
    ...
    #args = parser.parse_args(["--x", "\cos{t} + 2\cos{2t}", "--y", "\sin{t} - 2\sin{2t}", "--z", " -\sin{3t}", "--start", "0", "--end", "2\pi", "--num", "100", "--fps", "15", "--output", "output/trefoil_knot.gif",])
    x = ParametricCurve(latex2sympy(unescaped_str(args.x)), latex2sympy(unescaped_str(args.y)), latex2sympy(unescaped_str(args.z)))
    print(latex2sympy(unescaped_str(args.y)))
    data = np.linspace(float(latex2sympy(unescaped_str(args.start)).evalf()), float(latex2sympy(unescaped_str(args.end)).evalf()), args.num)
